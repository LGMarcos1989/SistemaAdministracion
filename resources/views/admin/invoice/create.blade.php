                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label for="invoice_date" class="block text-sm font-medium text-gray-700 mb-2">
                                        Fecha de la factura <span class="text-red-500"> *</span>
                                    </label>
                                    <div class="relative">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" id="invoice_number" name="invoice_number"  readonly value="{{ old('invoice_number',$propuestaFactura) }}"
                                            class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            ...
                            <div class="grid grid-cols-4  gap-6">
                                ...
                                <div class="col-span-1">
                                    <label for="tax_base" class="block text-sm font-medium text-gray-700 mb-2">
                                       Base Imponible<span class="text-red-500"> *</span>
                                    </label>
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <i class="fas fa-file-invoice-dollar text-gray-400"></i>
                                        </div>
                                        <input type="number" id="tax_base" name="tax_base" value="{{ old('tax_base') }}"
                                            class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                            placeholder="1000">
                                    </div>
                                    @error('tax_base')
                                        ...
                                        <select id="type_rate_id" name="type_rate_id" value="{{ old('type_rate_id') }}"
                                            ...
                                            <option selected disabled>Seleccione un Código</option>
                                             @foreach ($codigos as $c)
                                               <option value="{{ $c->id }}" data-rate="{{ $c->value }}">
                                                {{ $c->name.' '.$c->value.' %' }}
                                               </option>
                                           @endforeach
                                        </select>
...
{{-- Calcular el total de la factura, tres parametros intervienen:     tax_base ,  type_rate_id y  total --}}
   <script>
       
        const taxBaseInput = document.getElementById('tax_base');
        const typeRateInput = document.getElementById('type_rate_id');
        const totalInput = document.getElementById('total');

        function calcularTotalFactura() {
            const base = parseFloat(taxBaseInput.value) || 0;
            const selectedOption = typeRateInput.options[typeRateInput.selectedIndex];
            const porcentaje = parseFloat(selectedOption.dataset.rate) || 0;

            const total = base * (1 + porcentaje / 100);
        ...
        taxBaseInput.addEventListener('input', calcularTotalFactura);
        typeRateInput.addEventListener('change', calcularTotalFactura);
    </script>
@endpush
